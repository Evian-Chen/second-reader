...
namespace backend.Repository
{
    public class MeNotificationRepository : IMeNotificationRepository
    {
        ...
        {
            var book = await _context.UserBooks.Include(b => b.Book).FirstOrDefaultAsync(b => b.Id == userBookId);
        ...
        public async Task<NotificationDto> CreateOrderCreatedAsync(AppUser user, int orderId)
        {
            ...
            var content = $"您的訂單已成立。您訂購的書單如下：{orderBooksName}";
        ...
        {
            var book = await _context.UserBooks.Include(b => b.Book).FirstOrDefaultAsync(b => b.Id == userBookId);
            var content = $"您的上架書籍《{book!.Book!.Title}》已被 {user.AccountId} 下單！請盡速前往訂單管理中心確認。";

            var notification = new Notification
            {
                Title = Util.Literals.OrderRequest,
                Content = content,
        ...
        {
            var notifications = await _context.Notifications.Where(n => n.UnRead == unReadOnly && n.ReceiverAccountId == user.AccountId).ToListAsync();
            if (notifications == null) return null;
            return [.. notifications.Select(n => n.ToDtoFromModel())];
        }

        public async Task<NotificationDto?> GetNotificationById(int id)
        {
            var notification = await _context.Notifications.FirstOrDefaultAsync(c => c.Id == id);
